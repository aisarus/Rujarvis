...
/**
 * Сравнивать можно только то, что удалось прочитать.
 *
 * Шапка `ТЕКСТ_ОКНА` описывает ровно эту беду: «Полтора прогона ушло на поиск
 * несуществующей беды с печатью: печаталось-то всё, врало чтение». А код
 * подставлял вместо непрочитанного текста заглушку и сравнивал ЕЁ — и в
 * провалы попадало «window_write напечатал кириллицу», хотя не вышло чтение.
 */
function сравнить(прочитанное: string | null, условие: (текст: string) => boolean, имя: string): void {
  if (прочитанное === null) {
    console.log(`  нечем  ${имя} — окно не прочиталось, сравнивать не с чем`);
    return;
  }
  проверить(условие(прочитанное), имя);
}
...
if (напечатано !== null) console.log(`         в окне: «${напечатано}»`);
сравнить(напечатано, (текст) => текст.includes('Привет, мир'), 'кириллица дошла до окна буква в букву');

const буферПосле =
  (
    await шаг('прочитать буфер обмена', () =>
      запустить('osascript', ['-e', 'return the clipboard as text'], { timeout: 20_000 }),
    )
  )?.stdout.trim() ?? null;
if (буферПосле !== null) console.log(`         в буфере обмена: «${буферПосле}»`);
сравнить(буферПосле, (текст) => текст === сторож, 'буфер обмена вернулся к тому, что в нём было');
...
if (цель) {
  ...
  if (поле) {
    ...
    await подождать(1_000);
    const вОкне = await шаг('прочитать текст окна', () => текстОкна());
    if (вОкне !== null) console.log(`         в окне: «${вОкне}»`);
    сравнить(вОкне, (текст) => текст.includes('Привет из окна'), 'window_write напечатал кириллицу в найденное поле');

    await шаг('window_key backspace', () => инструменты.key(цель.pid, цель.windowId, 'backspace'));
    await подождать(800);
    const послеКлавиши = await шаг('прочитать текст окна', () => текстОкна());
    if (послеКлавиши !== null) console.log(`         в окне: «${послеКлавиши}»`);
    сравнить(послеКлавиши, (текст) => текст.endsWith('Привет из окн'), 'window_key дошла до окна — последний знак стёрт');
  }
