...
const CASES: Case[] = [
  ...
  // Сказанное во время работы: поправка, отдельная задача или мимо.
  {
    said: 'а пока найди мне картинки',
    check: (s) => (obviousAside(s)?.kind === 'task' ? null : 'не понято как вторая задача'),
  },
  {
    said: 'правка: шрифт крупнее',
    check: (s) => (obviousAside(s)?.kind === 'note' ? null : 'не понято как поправка'),
  },
  {
    said: 'спасибо',
