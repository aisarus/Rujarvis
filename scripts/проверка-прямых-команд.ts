...
function variants(said: string): { label: string; text: string }[] {
  const формы = [
    ...
    // Обычное слово рядом — ровно то, от чего команды разваливались.
    //
    // Замер 20.09.2026: из 265 команд каталога, обёрнутых одним обычным
    // словом, 160 переставали распознаваться вовсе и уходили агенту на
    // полминуты вместо мгновенного действия. Прибор без этих форм показывал
    // «всё прошло» на системе, где две трети вариантов не работали.
    { label: 'с наречием', text: `быстро ${said}` },
    { label: 'с частицей', text: `а теперь ${said}` },
    { label: 'просьбой', text: `можешь ${said}` },
  ];

  // Диктовка печатает сказанное дословно, поэтому слово рядом с ней — это
  // содержание, а не обёртка: «напечатай привет сейчас» набирает «сейчас».
  return said.startsWith('напечатай') || said.startsWith('печатай') || said.startsWith('введи')
    ? формы.slice(0, 3)
    : формы;
}
