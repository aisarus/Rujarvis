...
// Родитель основания — ПРЕДЫДУЩИЙ коммит main, а не сам main.
//
// Если сделать основание потомком головы, main окажется её предком, и GitHub
// ответит «No commits between …»: сравнивать нечего. От предыдущего коммита
// истории расходятся, и в diff попадает ровно кусок плюс изменения одного
// последнего коммита — небольшой шум, с которым можно жить.
const предыдущий = коммитГлавной.parents[0]?.sha ?? головаSha;
...
for (const [номер, кусок] of куски.entries()) {
  ...
  const коммит = await api('git/commits', 'POST', {
    message: `Основание для разбора куска ${номер + 1}: ${подпись}\n\nВетка не для слияния.`,
    tree: дерево2.sha,
    parents: [предыдущий],
  });

  try {
    await api('git/refs', 'POST', { ref: `refs/heads/${имя}`, sha: коммит.sha });
  } catch {
    await api(`git/refs/heads/${имя}`, 'PATCH', { sha: коммит.sha, force: true });
  }
