...
async function проверки(): Promise<Array<{ имя: string; итог: Итог }>> {
  ...
  // 5 и 6. На Маке — два вопроса, а не один.
  //
  // Первая версия спрашивала сразу «собрался ли скрипт и отозвался ли Терминал»
  // и упала в CI: `osascript` не вернулся за 30 секунд и не написал ничего. Так
  // macOS ведёт себя, когда событие Терминалу посылают без выданного
  // разрешения: система показывает «разрешить управлять Терминалом?» и ЖДЁТ
  // щелчка. На раннере щёлкнуть некому. У человека это окно появится, он
  // нажмёт «Разрешить», и вход пойдёт дальше.
  //
  // Поэтому вопросы разведены. «Собирается ли скрипт» — это наша строка, и на
  // него `osacompile` отвечает твёрдо и без всяких разрешений. «Отозвался ли
  // Терминал» — про машину, и на раннере честный ответ там «нечем мерить».
  if (process.platform !== 'darwin') {
    итоги.push({
      имя: 'macOS: AppleScript собирается',
      итог: { вид: 'нечем мерить', почему: `osacompile есть только на macOS, здесь ${process.platform}` },
    });
    итоги.push({
      имя: 'macOS: Терминал выполняет команду входа',
      итог: { вид: 'нечем мерить', почему: `osascript есть только на macOS, здесь ${process.platform}` },
    });
  } else {
    // Заглушка вместо CLI: настоящий вход поднял бы браузер за человека. Путь —
    // с пробелом и с апострофом: оба ломают строку оболочки, если кавычки
    // собраны неверно.
    const корень = mkdtempSync(path.join(os.tmpdir(), 'jarvis-login-mac-'));
    const папка = path.join(корень, "dva slova o'brien");
    mkdirSync(папка, { recursive: true });
    const метка = path.join(папка, 'mark.txt');
    const заглушка = path.join(папка, 'login-stub.sh');
    writeFileSync(заглушка, ['#!/bin/sh', `printf 'POZVALI %s' "$*" > "$(dirname "$0")/mark.txt"`, ''].join(NL), {
      encoding: 'utf8',
      mode: 0o755,
    });
    const запуск = терминалДляВхода(заглушка, ['auth', 'login'], папка);
    ...
    const собранный = path.join(корень, 'vhod.scpt');
    const сборка = await new Promise<{ код: number | null; ошибка: string }>((resolve) => {
      let ошибка = '';
      const дитя = spawn('osacompile', ['-e', скрипт, '-o', собранный], { stdio: ['ignore', 'ignore', 'pipe'] });
      дитя.stderr?.on('data', (к: Buffer) => {
        ошибка += к.toString('utf8');
      });
      дитя.on('error', (е: Error) => resolve({ код: null, ошибка: е.message }));
      дитя.on('exit', (код) => resolve({ код, ошибка }));
    });
    итоги.push({
      имя: 'macOS: AppleScript собирается',
      итог:
        сборка.код === 0
          ? { вид: 'прошло', чем: 'osacompile собрал скрипт входа без ошибок, путь с пробелом и апострофом' }
          : сборка.код === null
            ? { вид: 'нечем мерить', почему: `osacompile не запустился: ${сборка.ошибка.slice(0, 160)}` }
            : { вид: 'не прошло', почему: `скрипт не собирается: ${сборка.ошибка.trim().slice(0, 200)}` },
    });

    // 6. Настоящий запуск. Висящий osascript гасим сами: первая версия
    // оставляла его жить после проверки.
    let ошибка = '';
    ...
    const дошло = await ждатьФайл(метка, завис ? 1_000 : 20_000);
    const нетРазрешения = /-1743|not authoriz|Not authorized/u.test(ошибка);
    итоги.push({
      имя: 'macOS: Терминал выполняет команду входа',
      итог: дошло
        ? { вид: 'прошло', чем: `Терминал выполнил: ${readFileSync(метка, 'utf8').trim()}` }
        : нетРазрешения
          ? { вид: 'нечем мерить', почему: `нет разрешения на автоматизацию Терминала: ${ошибка.trim().slice(0, 120)}` }
          : завис && !ошибка.trim()
            ? {
                вид: 'нечем мерить',
                почему:
                  'osascript ждёт ответа 30 с и молчит — так macOS ждёт щелчка «разрешить управлять Терминалом», а на раннере щёлкнуть некому',
              }
            : {
                вид: 'не прошло',
