...
const APP_ALIASES: ReadonlyArray<readonly [readonly string[], string]> = [
  ...
  // 'edge', а не 'msedge': здесь таблица ЗАПУСКА, а совпадать надо с
  // названием программы («Microsoft Edge»), а не с именем процесса. Слова
  // «msedge» нет ни в одном названии, поэтому «открой эдж» не находило ничего,
  // хотя «открой edge» работало — случайно, своим же словом. Имя процесса
  // остаётся в таблице ОКОН ниже, там оно и нужно.
  [['эдж', 'эдже', 'едж', 'edge', 'майкрософт эдж', 'мсэдж', 'microsoft edge'], 'edge'],
  [['телеграм', 'телега', 'телеграмм', 'telegram'], 'telegram'],
  [['фаерфокс', 'файрфокс', 'фокс', 'firefox'], 'firefox'],
  [['блокнот', 'нотпад', 'notepad'], 'notepad'],
  // Ведём на 'calculator', а не на 'calc'. Замер 26.09.2026: 'calc' ТОЧНО
  // совпадает со словом «Calc» в «LibreOffice Calc» и выигрывает у настоящего
  // калькулятора, у которого совпадение лишь частичное. «Открой калькулятор»
  // открывало таблицу.
  [['калькулятор', 'калькулятр', 'calc', 'calculator'], 'calculator'],
