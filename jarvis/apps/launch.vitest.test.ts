...
describe('сайт и вкладка — не программа', () => {
  it.each(['Открой мне сайт inbar', 'Открой сайт ютуб', 'открой сайт гугл', 'open the website inbar'])(
    '«%s» не ищется среди ярлыков «Пуска»',
    (фраза) => {
      // Живой прогон 26.09.2026: «Открой мне сайт inbar» совпало ТОЧНО по слову
      // «сайт» с ярлыком «Сайт PDFgear в Интернете», который оставил установщик
      // PDFgear, — и любая просьба открыть сайт открывала pdfgear.com. Сайт —
      // это браузер, его разбирает агент.
      expect(spokenTarget(фраза)).toBeNull();
      expect(matchAppLaunch(фраза)).toBeNull();
    },
  );

  it.each(['Ну какой PDF dir? Закрой вкладку', 'Закрой вкладку PDFDIR', 'закрой вкладки', 'close the tab'])(
    '«%s» не ищет запущенную программу по имени «вкладку»',
    (фраза) => {
      // Тот же прогон: «Закрой вкладку PDFDIR» искало среди запущенных
      // программу «вкладку pdfdir» и, не найдя, обрывало фразу.
      expect(spokenCloseTarget(фраза)).toBeNull();
    },
  );

  it('обычные программы по-прежнему запускаются и ищутся', () => {
    // Сторож не должен съесть соседей: слово «сайт» ловится, а имена программ —
    // нет. Edge идёт в `start` своим именем для запуска, блендер — поиском.
    expect(matchAppLaunch('Открой Edge')?.target).toBe('msedge');
    expect(matchAppLaunch('открой блокнот')?.target).toBe('notepad');
    expect(spokenTarget('открой блендер')).toBe('блендер');
    expect(spokenCloseTarget('закрой хром')).toBe('хром');
  });
});
