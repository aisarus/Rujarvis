...
export class DarwinWindowTools {
  ...
  private async дождатьсяДерева(): Promise<void> {
    ...
    while (Date.now() < конец) {
      ...
      // Выходим только после того, как дерево ХОТЬ РАЗ выросло.
      //
      // «Две подряд дали одинаковую длину» само по себе не признак готовности:
      // пока Chromium дерево не построил, оно одинаково коротко — одна рама
      // окна, — и ожидание кончалось раньше времени. Прогон на маке 26.09.2026
      // поймал это гонкой: два ответа по 12 элементов совпали, выход
      // состоялся, а страницы в дереве не было. Прошлый прогон прошёл случайно.
