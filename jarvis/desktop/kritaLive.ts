 ...
 *
 * Занятый и мёртвый выглядят одинаково: отметка не обновляется. Разница в том,
 * что занятый рано или поздно тикнет. Пятнадцать секунд отличают одно от
 * другого и не заставляют ждать всерьёз, когда сеанса правда нет.
 */
...
export async function sendLive(code: string): Promise<LiveResult> {
  ...
  // Не хоронить с первого взгляда.
  //
  // Отметка жизни стареет не только когда сеанс умер, но и когда он ЗАНЯТ:
  // слушатель на таймере, а предыдущий скрипт считается на том же потоке.
  // Тяжёлый модификатор — и следующая команда получала «живой Криты нет»
  // при работающем окне. Ждём немного: занятый отзовётся, мёртвый нет.
  ...
  while (Date.now() < until) {
    ...
    // Молчание отметки жизни во время работы — это НЕ смерть.
    //
    // Слушатель живёт на таймере Qt, а скрипт исполняется на том же потоке:
    // пока он считает, таймер не тикает и отметка стареет. Первый же тяжёлый
    // скрипт (стилизация фото, 590 тысяч пикселей) выглядел как «Крита
    // закрылась, не ответив» — при живом окне, которое честно работало.
    //
    // Признак принятия — исчезнувший файл команды: плагин снимает его ДО
    // исполнения. Пока команда не принята, отметка жизни ещё о чём-то
    // говорит; после — уже нет, и ждать надо до потолка времени.
    const принято = !existsSync(command);
    if (!принято && !isLive()) {
