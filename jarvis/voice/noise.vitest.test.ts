...
/**
 * Выдумка распознавателя не должна становиться работой.
 *
 * Список ловил точную фразу «продолжение в следующей серии» и пропускал «в
 * следующей части» — в живом логе есть обе: двадцать раз первая, дважды
 * вторая. «Субтитры подогнал Симон» доходила до разбора и заводила разговор.
 */
describe('выдумки распознавателя отсеиваются целиком', () => {
  it.each([
    'Смотрите продолжение в следующей серии',
    'Смотрите продолжение в следующей части',
    'Субтитры подогнал Симон',
  ])('«%s» — не речь', (сказано) => {
    expect(meaningfulSpeech(сказано)).toBeNull();
  });

  // Граница: просьба сделать субтитры — настоящая просьба, и её терять нельзя.
  it.each([
    'сделай субтитры к ролику',
    'добавь субтитры в видео',
  ])('«%s» — речь', (сказано) => {
    expect(meaningfulSpeech(сказано)).not.toBeNull();
  });
});

describe('выдумка в начале — шум, слово в середине — речь', () => {
  /**
   * Замечание CodeRabbit (кусок 4, PR №43). «корректор» ловился вхождением,
   * и «открой корректор» молча пропадало: человек видел, что команда не
   * выполнена, а причины не узнавал.
   */
  it('титры Whisper по-прежнему отбрасываются', () => {
    expect(meaningfulSpeech('Корректор А. Егорова')).toBeNull();
  });

  it('команда со словом «корректор» доходит до разбора', () => {
    expect(meaningfulSpeech('открой корректор')).not.toBeNull();
    expect(meaningfulSpeech('позови корректора')).not.toBeNull();
  });
});
