...
describe('вопросительная интонация не глушит красную линию', () => {
  /**
   * Замечание CodeRabbit (кусок 4, PR №43). `tokenize` доставляет '?'
   * отдельным токеном — он несёт интонацию, а не слово. В разборе управления
   * осмысленная часть превращалась в «хватит ?», ни с чем не совпадала, и
   * остановка не срабатывала. Человек в этот момент говорит неуверенно —
   * «Джарвис, хватит?» — то есть ровно тогда, когда остановка нужнее всего.
   */
  it.each(['Джарвис, хватит?', 'стоп?', 'Тишина?', 'хватит?', 'пауза?'])(
    '«%s» доходит до управления',
    (фраза) => {
      expect(matchVoiceControl(фраза)).not.toBeNull();
    },
  );

  it('обычный вопрос управлением не становится', () => {
    expect(matchVoiceControl('ты уже закончил?')).toBeNull();
    expect(matchVoiceControl('а когда остановишь сервис после тестов?')).toBeNull();
  });
});

describe('Джарвис не должен перебивать сам себя', () => {
  /**
   * Замечание CodeRabbit (кусок 4, PR №43). Ответ на паузу был «Пауза.» — то
   * же слово, что и команда. Эхо-страж спрашивает разбор управления ПЕРВЫМ,
   * поэтому собственный голос из колонок за свой не считался: управление
   * срабатывало второй раз, пауза на уже остановленной задаче не вставала, и
   * человек слышал «нечего ставить на паузу», хотя она стоит.
   *
   * Проверяется не одна фраза, а правило: НИ ОДИН ответ на управление не
   * должен сам читаться как управление. Новый ответ, нарушивший его, валит
   * набор.
   */
  it('ни один ответ на управление не читается как команда', () => {
    ...
    const команды: VoiceControl[] = ['stop', 'cancel', 'pause', 'resume', 'mute'];
    const ответы: string[] = [];
    for (const цель of [было, нечего]) {
      for (const control of команды) {
        ответы.push(applyVoiceControl({ control, phrase: control }, цель).spoken);
      }
    }

    const виноватые = ответы.filter((ответ) => matchVoiceControl(ответ) !== null);
