...
export interface SelfHero {
   ...
   * `null` — не измерено.
   *
   * Игра присылает частичный блок героя посреди живой партии, и подстановка
   * нуля превращала «неизвестно» в «измерено и равно нулю»: порог опасности
   * видел здоровье 0% и кричал «один вплотную», советчик принимал это за миг
   * гибели и глушил предупреждение, а оверлей показывал «выкуп готов» за ноль
   * золота. Координаты здесь обязательны: без них герой вообще не собирается.
   */
  level: number | null;
  alive: boolean;
  /** Здоровье в процентах, 0..100. `null` — не измерено. */
  hp: number | null;
  x: number;
  y: number;
...
export function readPacket(сырой: unknown, at: number = Date.now()): DotaPacket | null {
  ...
  if (геройСырой && typeof геройСырой.name === 'string') {
    ...
    // Герой без координат — не повод выбросить весь пакет.
    //
    // 21.09.2026 посреди живой игры вид перестал обновляться: журнал рос, а
    // окно ушло, решив, что помощник умер. Причина была здесь — `return null`
    // на весь снимок, если у героя нет `xpos`. А в пакете кроме героя есть
    // часы, золото, предметы, события и вся миникарта.
    //
    // Теперь теряется только сам герой, и пороги честно отвечают «нечем
    // мерить» вместо того, чтобы молчать без объяснения.
    if (x === null || y === null) свой = null;
    // Неизвестное остаётся НЕИЗВЕСТНЫМ, а не нулём.
    //
    // Частичный блок героя бывает посреди живой игры, и `?? 0` превращал
    // «не измерено» в «измерено и равно нулю». Последствия дорогие: порог
    // опасности видел здоровье 0% и кричал «один вплотную», советчик принимал
    // это за миг гибели и ГЛУШИЛ предупреждение, а оверлей показывал «выкуп
    // готов» за ноль золота.
    else свой = {
      hero: геройСырой.name,
      level: число(геройСырой.level),
      alive: геройСырой.alive !== false,
      hp: число(геройСырой.health_percent),
      x,
      y,
      buybackCost: число(геройСырой.buyback_cost),
      buybackCooldown: число(геройСырой.buyback_cooldown),
    };
  }
