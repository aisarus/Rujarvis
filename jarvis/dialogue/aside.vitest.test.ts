...
describe('вопрос модели', () => {
  ...
  it('требует ответа одним словом', () => {
    // Проверяем ТРЕБОВАНИЕ, а не слова, которые и так есть в вопросе.
    //
    // Раньше совпадение давали «правка» и «отдельно» из самого текста
    // вопроса: убери строку с требованием — и проверка осталась бы зелёной.
    expect(askAbout('что-нибудь', 'Работа')).toContain('одним словом');
  });
});
