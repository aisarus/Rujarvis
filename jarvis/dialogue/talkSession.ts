 ...
 *
 * Не из вежливости и не по обещанию в запросе: разрешён только список
 * `TALK_TOOLS` — глаголы работы, и ничего больше. Числом их здесь нарочно не
 * называем: список рос (после первой записи добавились `add_step` и
 * `work_now`), а число в комментарии не росло, и читатель, сверявший одно с
 * другим, получал ложную картину.
 *
 * Строго говоря, `--allowedTools` разрешает перечисленное, а не отключает
 * встроенные средства чтения самого CLI. Граница здесь в другом: папка — дом
 * Джарвиса, а не проект человека, а режим разрешений `default` в безголовом
 * запуске молча отказывает всему, чего нет в списке.
 *
...
export class TalkSession {
   ...
   *
   * Нужно потому, что подъём идёт с ожиданиями: пока он шёл, человек мог
   * сказать «забудь» или Джарвис — закрыться. Раньше поднятая после этого
   * сессия оставалась без хозяина: ссылка на неё была только в поле, которое
   * уже обнулили, а процесс CLI продолжал жить.
   */
  private поколение = 0;
  ...
  async hear(said: string): Promise<void> {
    ...
    //
    // Фраз в работе может быть несколько сразу, поэтому хранится НАБОР, а не
    // одна: раньше завершение хода по «А» снимало метку с идущего хода по
    // «Б», и повтор «Б» всё-таки вставал в очередь. И метка ставится ДО
    // подъёма: два одинаковых повтора, сказанных пока сессия поднимается,
    // оба проходили проверку.
    if (this.думаетНад.has(фраза)) {
      this.log(`повтор, уже думаю: ${фраза}`);
      return;
    }
    this.думаетНад.add(фраза);
    ...
    try {
      ...
      // первая фраза могла прозвучать через час: второй ход тогда прикладывал
      // всё, что случилось с запуска, то есть повторял первый.
      const первый = !live.hasSpoken();
      if (первый) this.delta = new WorkDelta(this.options.delta);
      ...
      // Молчание здесь неотличимо от поломки: человек сказал и не услышал
      // ничего. Раньше сбой подготовки хода уходил только в журнал.
      this.log(`ход не удался: ${message(error)}`);
      await this.say('Не смог ответить.');
      return;
    } finally {
      this.думаетНад.delete(фраза);
    }
    ...
    if (!result.ok) {
      ...
      //
      // Признак берётся из самого результата, а не из текста ошибки: по слову
      // «отмен» пряталась бы и настоящая беда, а отмена с другой формулировкой
      // («Человек попросил забыть») вызывала бы извинение на пустом месте.
      if (!result.cancelled) {
  ...
  private async raise(): Promise<TalkLive | null> {
    ...
    const key: SessionKey = {
      ...
      // всему, чего нет в списке. Для разговора это и нужно: ровно перечисленные глаголы и
      // ничего больше.
    ...
    // Пока поднимались, могли попросить забыть или закрыться. Тогда эта
    // сессия уже никому не нужна, и держать её некому.
    if (this.поколение !== моё) {
      live.dispose('подъём отменён');
      return null;
    }
