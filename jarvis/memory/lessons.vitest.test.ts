...
describe('подходы считаются промежутком, а не часами', () => {
  /**
   * Замечание CodeRabbit (кусок 3, PR №42). Время делилось на жёсткие
   * десятиминутки, и две неудачи в 10:09:30 и 10:10:30 давали два подхода
   * вместо одного — разовая стена лезла в промпт как урок.
   */
  it('две неудачи через минуту — это один подход, даже на границе часов', () => {
    const край = Math.ceil(AT / 600_000) * 600_000;
    const lessons = lessonsFrom({ events: [error('стена', край - 30_000), error('стена', край + 30_000)] });
    expect(lessons[0]?.occasions).toBe(1);
  });

  it('две неудачи через полчаса — это два подхода', () => {
    const lessons = lessonsFrom({ events: [error('стена', AT), error('стена', AT + 30 * 60_000)] });
    expect(lessons[0]?.occasions).toBe(2);
  });
});
