...
describe('сказанное целиком, как человек и говорит', () => {
  // Сквозная проверка всех 62 обещанных команд, 20.09.2026. Каждая из них
  // работает слово в слово — и разваливается от одного естественного слова
  // рядом. Человек не заучивает справочник: он говорит «сверни окно», а не
  // «сверни».
  //
  // Цена у промаха разная, и худшие здесь не те, где ничего не произошло.

  // «Сверни окно» доходило до слоя закрытия и ЗАКРЫВАЛО окно: свернуть просили,
  // а работу потеряли. Необратимое действие вместо обратимого — худшее, что
  // может сделать голосовой помощник.
  it.each([
    ['сверни окно', 'key:win+down'],
    ['сверни это окно', 'key:win+down'],
    ['разверни окно', 'key:win+up'],
    ['разверни это окно', 'key:win+up'],
    ['закрой это окно', 'key:alt+f4'],
  ])('«%s» → %s', (phrase, expected) => {
    expect(act(phrase)).toBe(expected);
  });

  // «Открой новую вкладку» уходило в слой запуска, и Джарвис искал в меню
  // «Пуск» программу с названием «новую вкладку».
  it.each([
    ['открой новую вкладку', 'key:ctrl+t'],
    ['закрой эту вкладку', 'key:ctrl+w'],
    ['вернись на предыдущую вкладку', 'key:ctrl+shift+tab'],
    ['вернись к предыдущей вкладке', 'key:ctrl+shift+tab'],
  ])('«%s» → %s', (phrase, expected) => {
    expect(act(phrase)).toBe(expected);
  });

  // «Кликни правой кнопкой» превращалось в поиск кнопки с названием «правой
  // кнопкой» — то есть в клик неизвестно куда или в уход к агенту.
  it.each([
    ['кликни правой кнопкой', 'click:right'],
    ['нажми правой кнопкой', 'click:right'],
    ['правой кнопкой мыши', 'click:right'],
    ['двойной щелчок', 'click:left:2'],
    ['щелкни дважды', 'click:left:2'],
  ])('«%s» → %s', (phrase, expected) => {
    expect(act(phrase)).toBe(expected);
  });

  // «Нажми стрелку вниз» искалось как кнопка с названием «стрелку вниз».
  it.each([
    ['нажми стрелку вниз', 'key:down'],
    ['стрелка вверх', 'key:up'],
    ['нажми стрелку влево', 'key:left'],
    ['стрелку вправо', 'key:right'],
  ])('«%s» → %s', (phrase, expected) => {
    expect(act(phrase)).toBe(expected);
  });

  it.each([
    ['прокрути страницу вниз', 'scroll:-3'],
    ['прокрути страницу вверх', 'scroll:3'],
    ['пролистай вниз', 'scroll:-3'],
    ['пролистай вверх', 'scroll:3'],
    ['страницу вниз', 'key:pagedown'],
    ['страницу вверх', 'key:pageup'],
  ])('«%s» → %s', (phrase, expected) => {
    expect(act(phrase)).toBe(expected);
  });

  it.each([
    ['обнови страницу', 'key:f5'],
    ['отмени последнее действие', 'key:ctrl+z'],
    ['скопируй это', 'key:ctrl+c'],
    ['вставь сюда', 'key:ctrl+v'],
  ])('«%s» → %s', (phrase, expected) => {
    expect(act(phrase)).toBe(expected);
  });

  // Вопросы о работе человек задаёт со словом «ты», а в таблице его не было.
  it('«чем ты занят» открывает окно работы', () => {
    expect(parseDirectCommand('чем ты занят')).toEqual({ kind: 'log', on: true });
    expect(parseDirectCommand('покажи что ты делаешь')).toEqual({ kind: 'log', on: true });
  });

  it('«на каком ты шаге» отвечает о плане', () => {
    expect(parseDirectCommand('на каком ты шаге')).toEqual({ kind: 'where' });
  });

  it('«что умеешь» показывает список', () => {
    expect(parseDirectCommand('что умеешь')).toEqual({ kind: 'help', on: true });
    expect(parseDirectCommand('какие есть команды')).toEqual({ kind: 'help', on: true });
  });

  it('«не показывай окна» уводит в фон', () => {
    expect(parseDirectCommand('не показывай окна')).toEqual({ kind: 'mode', show: false });
  });

  // Граница между слоями. «Поставь на паузу» — просьба отложить РАБОТУ, и
  // слова остановки разбираются раньше этой таблицы. Забрать её сюда значило
  // бы на просьбу подождать переключить музыку.
  it('«поставь на паузу» таблица не забирает', () => {
    expect(parseDirectCommand('поставь на паузу')).toBeNull();
  });
});

describe('«ещё раз» — то же самое ещё раз', () => {
  // «Прокрути вниз ещё раз» — самая частая фраза при чтении страницы, и она не
  // разбиралась вовсе: счётчик повторов требовал числа, а «ещё» числом не
  // является. Тридцать секунд через агента за один щелчок колеса.
  it.each([
    ['прокрути вниз еще раз', 'scroll:-3'],
    ['прокрути вниз ещё раз', 'scroll:-3'],
    ['вниз еще раз', 'key:down'],
    ['нажми enter еще раз', 'key:enter'],
    ['кликни еще раз', 'click:left'],
  ])('«%s» → %s', (phrase, expected) => {
    expect(act(phrase)).toBe(expected);
  });

  // Граница: продиктованный текст не теряет своих слов.
  it('в диктовке «ещё раз» остаётся текстом', () => {
    expect(parseDirectCommand('напечатай привет еще раз')).toEqual({
      kind: 'type',
      text: 'привет еще раз',
    });
  });

  // Граница: без команды впереди повторять нечего.
  it('одинокое «ещё раз» командой не становится', () => {
    expect(parseDirectCommand('еще раз')).toBeNull();
    expect(parseDirectCommand('открой блендер еще раз')).toBeNull();
  });
});

describe('обычные просьбы остаются работой для агента', () => {
  ...
  it.each([
    'найди отчет за март',
    'сохрани отчет в папку загрузки',
  ...
  ])('«%s» уходит агенту', (phrase) => {
    expect(parseDirectCommand(phrase)).toBeNull();
  });
});

describe('оверлей в игре', () => {
  it.each([
    'оверлей',
    'включи оверлей',
    'покажи оверлей',
    'подсказки',
    'включи подсказки',
  ])('«%s» включает картинку и голос', (phrase) => {
    expect(parseDirectCommand(phrase)).toEqual({ kind: 'dotaOverlay', mode: 'full' });
  });
  ...
  it.each([
    ...
    'только на экране',
    'подсказки на экран',
  ])('«%s» оставляет картинку без голоса', (phrase) => {
    expect(parseDirectCommand(phrase)).toEqual({ kind: 'dotaOverlay', mode: 'silent' });
  });
  ...
  it.each([
    ...
    'выключи подсказки',
    'убери подсказки',
  ])('«%s» убирает всё', (phrase) => {
    expect(parseDirectCommand(phrase)).toEqual({ kind: 'dotaOverlay', mode: 'off' });
  });

  it('фраза с довеском уходит агенту, а не включает оверлей', () => {
    // Общее правило дома: совпадение должно быть точным. «Подсказки по сборке
    // против Лича» — это вопрос, а не выключатель.
    expect(parseDirectCommand('подсказки по сборке против лича')).toBeNull();
  });
});

describe('аварийный выключатель', () => {
  it.each(['убейся', 'умри', 'сдохни', 'убей агента', 'убей все процессы'])(
    '«%s» убивает процессы агента',
    (phrase) => {
      expect(parseDirectCommand(phrase)).toEqual({ kind: 'selfDestruct' });
    },
  );

  it('фраза с довеском уходит агенту', () => {
    // «Убей процесс блокнота» — работа, а не аварийный выключатель.
    expect(parseDirectCommand('убей процесс блокнота')).toBeNull();
  });
});
 ...
 * Настройки голосом.
 *
 * Настройки живут в окне, окно открывается мышью — а человек, ради которого
 * это делается, мышью может не владеть. Громкость, скорость и размер должны
 * меняться голосом и без единого клика.
 */
describe('настройки голосом', () => {
  it.each([
    ['говори громче', 'speechVolume', 'up'],
    ['говори тише', 'speechVolume', 'down'],
    ['говори медленнее', 'speechSpeed', 'down'],
    ['помедленнее', 'speechSpeed', 'down'],
    ['плохо вижу', 'bigMode', 'on'],
    ['крупнее', 'bigMode', 'on'],
    ['обычный шрифт', 'bigMode', 'off'],
    ['не отзывайся', 'wakeAck', 'off'],
    ['выключи микрофон', 'mic', 'off'],
    ['speak slower', 'speechSpeed', 'down'],
  ])('«%s» меняет %s в сторону %s', (фраза, what, direction) => {
    expect(parseDirectCommand(фраза)).toEqual({ kind: 'setting', what, direction });
  });

  /**
   * «Громче» — это системный звук, и отнимать его нельзя.
   *
   * Человек, сказавший «громче», имеет в виду музыку или видео, а не речь
   * Джарвиса. Поэтому у речи фразы длиннее и однозначнее.
   */
  it('«громче» осталось системной громкостью', () => {
    expect(parseDirectCommand('громче')).toEqual({ kind: 'key', keys: 'volumeup' });
    expect(parseDirectCommand('сделай тише')).toEqual({ kind: 'key', keys: 'volumedown' });
  });

  it('включить микрофон голосом нельзя — и такой команды нет', () => {
