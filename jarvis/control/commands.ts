...
const FILLER = [
  ...
  'вот', 'уже', 'вообще',
  // Наречия образа действия: к тому, ЧТО сделать, они не добавляют ничего.
  // Замер 20.09.2026: из 265 команд каталога, обёрнутых обычным словом, 160
  // переставали распознаваться вовсе — то есть уходили агенту на полминуты
  // вместо мгновенного действия. «Быстро сверни» это та же «сверни».
  'быстро', 'быстренько', 'просто', 'короче',
];

/**
 * Слова, которые ничего не значат ТОЛЬКО в начале фразы.
 *
 * Отдельно от FILLER, и это не педантизм. «А» и «и» — однобуквенные слова, и
 * они же имена клавиш: выбросив «а» где угодно, мы превратим «нажми а» в
 * «нажми» и потеряем нажатие. В начале фразы клавишей они быть не могут, там
 * это частица: «а теперь сверни», «и закрой окно».
 *
 * «Можешь» и «может» здесь по другой причине: вежливая просьба — та же
 * команда, и отвечать на неё через агента значит платить тридцать секунд за
 * вежливость.
 */
const LEADING_FILLER = ['а', 'и', 'можешь', 'может', 'слушай', 'слышь'];
...
function normalise(utterance: string): string {
  ...
  // Частицы в начале — обращение к вниманию, а не часть команды.
  while (words.length > 1 && LEADING_FILLER.includes(words[0] as string)) {
    words = words.slice(1);
  }
