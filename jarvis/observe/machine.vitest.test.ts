...
describe('сбылось ли ожидание', () => {
  ...
  it('смена заголовка засчитана', () => {
    // Впереди в обоих снимках ОДНО И ТО ЖЕ окно, а меняется заголовок
    // фонового. Раньше здесь менялось и то и другое сразу, и проверка
    // оставалась зелёной, даже если разбор перестал замечать переименования:
    // засчитывалась смена переднего окна.
    const было2 = снимок('Блокнот', [
      ['Блокнот', 'notepad'],
      ['Почта — Edge', 'msedge'],
    ]);
    const стало2 = снимок('Блокнот', [
      ['Блокнот', 'notepad'],
      ['Карты — Edge', 'msedge'],
    ]);
    const d = diff(было2, стало2);
    expect(d.renamed).toEqual([{ was: 'Почта — Edge', now: 'Карты — Edge' }]);
    expect(d.front).toBeUndefined();
    expect(unmet({ titleChanged: true }, стало2 && d, стало2)).toBeNull();
  });

  it('переключение между открытыми окнами — не смена заголовка', () => {
    // Оба окна уже открыты, ни один заголовок не менялся. Раньше `front`
    // один засчитывал успех, и проверка смены вкладки принимала за неё
    // обычное переключение окна.
    const было2 = снимок('Блокнот', [
      ['Блокнот', 'notepad'],
      ['Почта — Edge', 'msedge'],
    ]);
    const стало2 = снимок('Почта — Edge', [
      ['Блокнот', 'notepad'],
      ['Почта — Edge', 'msedge'],
    ]);
    expect(unmet({ titleChanged: true }, diff(было2, стало2), стало2)).toContain('не сменился');
  });

  it('закрытие одного из двух одинаковых окон замечается', () => {
    // Заголовки складывались в множество, и повторы схлопывались: закрытие
    // одного из двух «Документов» давало пустую разницу, а проверка
    // «ничего не должно было случиться» его не видела.
    const было2 = снимок('Документ', [
      ['Документ', 'notepad'],
      ['Документ', 'notepad'],
    ]);
    const стало2 = снимок('Документ', [['Документ', 'notepad']]);
    expect(diff(было2, стало2).closed).toEqual(['Документ']);
    expect(unmet({ nothing: true }, diff(было2, стало2), стало2)).toContain('закрылось');
  });

  it('смена вкладки — это действие, а не бездействие', () => {
    const было2 = снимок('Блокнот', [
      ['Блокнот', 'notepad'],
      ['Почта — Edge', 'msedge'],
    ]);
    ...
    expect(unmet({ nothing: true }, diff(было2, стало2), стало2)).toContain('вместо');
  });
