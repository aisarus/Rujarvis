...
describe('план проб выдаёт столько нажатий, сколько обещал', () => {
  /**
   * Замечание CodeRabbit (кусок 3, PR №42). Предел пересчитывался в условии
   * цикла, а длина плана росла на каждом шаге: нажатий выходило примерно
   * вдвое меньше, недоданные уходили в «подвигать мышью», на которое страница
   * отвечает редко, — и живая вещь могла получить провал.
   */
  it.each([
    [{ handles: 5, probes: 12 }, 5],
    [{ handles: 50, probes: 8 }, 4],
    [{ handles: 2, probes: 12 }, 2],
  ])('%o → %i нажатий', (вход, сколько) => {
    const нажатий = probePlan(вход).filter((шаг) => шаг.what === 'нажать').length;
    expect(нажатий).toBe(сколько);
  });
});
