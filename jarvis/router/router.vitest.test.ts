...
describe('правка того, что на экране, — это управление машиной', () => {
  it.each([
    'Стереть весь текст в адресной строке Edge — выделить всё и удалить.',
    'Сотри весь текст',
    'Сотри всё',
    'Выдели всё и удали',
    'Очисти адресную строку',
    'Впиши в поле поиска слово кот',
  ])('«%s» просит computer', (utterance) => {
    // Снято с живого прогона 26.09.2026 у чужого человека. Эти фразы получали
    // browser и files, но НЕ computer — значит инструменты окон и клавиш не
    // попадали в разрешённые, CLI отказывал сам, и агент честно докладывал:
    // «нужно разрешить инструменты jarvis-desktop». Включить их человеку было
    // нечем: виновата классификация, а не разрешения. Адресная строка Edge так
    // и осталась нетронутой.
    expect(route(utterance).needs).toContain('computer');
  });

  it.each([
    'переключись на сафари',
    'закрой сафари',
    'открой фаерфокс',
  ])('«%s» — это окно человека, а не наш Playwright', (utterance) => {
    // Chrome и Edge в словаре были, а Safari, Firefox и Opera — нет. У тестера
    // на маке Safari стоит по умолчанию, и без этого «переключись на сафари»
    // осталось бы без права трогать окна.
    expect(route(utterance).needs).toContain('computer');
  });
});
...
describe('вопрос — это вопрос, а не приказ', () => {
  const intent = (said: string) =>
    route(said, { basePermissions: { canRunCode: true } as never }).intent;
  const risk = (said: string) =>
    route(said, { basePermissions: { canRunCode: true } as never }).risk;

  // Живой случай 20.09.2026: человек сказал, что Джарвис «не понимает
  // концепцию вопросов». Вопрос про экран понимался как приказ открыть
  // программу, а вопрос про книгу — как общение с людьми, и Джарвис просил
  // разрешения на ответ.
  it('вопрос про экран — посмотреть и ответить, а не открывать', () => {
    expect(intent('что это за окно')).toBe('query_screen');
    expect(intent('что сейчас на экране')).toBe('query_screen');
    expect(intent('какая это программа')).toBe('query_screen');
  });

  it('вопрос про мир — разговор, и не чувствительный', () => {
    expect(intent('кто написал войну и мир')).toBe('chat');
    expect(risk('кто написал войну и мир')).toBe('safe');
    expect(intent('кто написал этот роман')).toBe('chat');
    expect(intent('когда построили эйфелеву башню')).toBe('chat');
    expect(intent('сколько будет двести на триста')).toBe('chat');
  });

  // Граница: просьба остаётся просьбой, даже если начинается похоже.
  it('приказ не превращается в вопрос', () => {
    expect(intent('открой блокнот')).toBe('open_app');
    expect(intent('напиши письмо Ивану')).toBe('communicate');
    expect(intent('создай сферу в блендере')).toBe('make');
  });

  // Граница: вопрос с просьбой внутри — всё-таки работа.
  it('вопрос с поручением внутри остаётся работой', () => {
    expect(intent('можешь открыть блокнот')).toBe('open_app');
    expect(intent('а сделай мне сферу')).toBe('make');
  });
});
