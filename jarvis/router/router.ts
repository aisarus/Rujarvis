 ...
 */
/**
 * Глаголы, которыми спрашивают, а не поручают.
 *
 * «Ты понял», «ты видишь», «ты помнишь» — это проверка, обращённая к
 * собеседнику. Поручением ни один из них быть не может: человек не сообщает
 * помощнику, что тот что-то понял.
 *
 * Список закрытый и маленький нарочно. Общее правило «фраза начинается с ты —
 * значит вопрос» сюда не годится: «ты должен сделать крышу синей» начинается
 * так же, а это поправка.
 */
const CHECKING_VERBS = [
  'понял', 'поняла', 'понимаеш', 'понимает',
  'видиш', 'видел', 'видиш', 'слышиш', 'слышал',
  'помниш', 'знаеш', 'уверен', 'читал', 'смотрел', 'запомнил',
];

/**
 * Обращённая к Джарвису проверка: «ты понял что надо делать?»
 *
 * Отдельно от знака вопроса, и это вынужденно. Замер 20.09.2026 на настоящем
 * распознавателе: эту фразу large-v3-turbo записывает с ТОЧКОЙ — все три
 * испробованных варианта. Знака, на который опирается всё остальное, здесь
 * просто нет, а человек спрашивал именно так и получил «Учту» и тишину.
 */
function checksOnUs(tokens: readonly string[]): boolean {
  const first = tokens[0];
  if (first !== 'ты' && first !== 'вы') return false;
  return tokens.slice(1, 4).some((token) =>
    CHECKING_VERBS.some((stem) => token.startsWith(stem)),
  );
}

function asksSomething(tokens: readonly string[]): boolean {
  // Знак вопроса приезжает отдельным словом — см. `tokenize`. Раньше он
  // проверялся отдельным параметром `raw`, и это был второй механизм на то же
  // самое: в поле `asks` его никто не передавал, и «ты понял что надо делать?»
  // вопросом не считалось. Один путь вместо двух.
  if (tokens.includes('?')) return true;
  if (checksOnUs(tokens)) return true;
