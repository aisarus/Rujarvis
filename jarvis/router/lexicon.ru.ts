...
export const CAPABILITY_RULES: CapabilityRule[] = [
  {
    ...
    stems: [
      ...
      // Правка того, что уже на экране, — это тоже управление машиной.
      //
      // Живой прогон 26.09.2026: «Стереть весь текст в адресной строке Edge —
      // выделить всё и удалить» получила browser и files, но НЕ computer.
      // Инструментов окон у задачи не было, агент честно потянулся к
      // focus_window, window_list и press_key, получил отказ и доложил
      // человеку «нужно разрешить инструменты jarvis-desktop» — то есть
      // отправил его включать то, чего включить нельзя. Виновата была
      // классификация, а не разрешения.
      // «Сотри» сводится к основе «сотр», а не «стер»: у этого глагола
      // чередование в корне, и одной основы на все его формы нет.
      'стер', 'стир', 'сотр', 'удал', 'выдел', 'очист', 'замен', 'впиш', 'введ',
      'строк', 'адресн', 'поле', 'полей',
  ...
  },
  {
    ...
    stems: [
      ...
      'сафари', 'safari', 'фаерфокс', 'файрфокс', 'опера', 'opera',
...
export const REFERENTIAL_STEMS = [
  ...
  // Отсылка не словом, а самим смыслом: «почему не получилось?», «что вышло?»,
  // «как прошло?» — про только что сделанное, хотя ни «это», ни «тот» не
  // сказано. Без этих основ такой вопрос отвечался без контекста, то есть
  // уверенно и ни о чём.
  'получ', 'вышл', 'сработа', 'удалос', 'прошл',
];
