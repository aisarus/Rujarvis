  ...
  if (request.outputDir) {
    parts.push(
      [
        ...
        `Внутри разделы: ${outputSectionNames().join(', ')}. Раздел выбирается по типу`,
        'файла сам — вызови move_to_output, и файл окажется где нужно. Все файлы одной',
        'задачи передавай с одним и тем же topic — коротким названием задачи на языке',
        'человека: они лягут подпапкой внутри раздела. Разделов сам не создавай; что',
        'осталось в корне, после работы разложится по разделам без тебя.',
