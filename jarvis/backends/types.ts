...
export interface BackendResult {
  ...
  usageLimited?: boolean;
  /**
   * Что процесс написал в stderr — для разбора, не для человека.
   *
   * Обычно он не нужен: причина уже пришла из потока событий. Но когда она
   * пришла и оказалась бесполезной, настоящая часто лежит именно здесь.
   * Живой случай: агент три минуты молчал и выдал «401 API key is invalid» —
   * при том, что ключей в системе нет, — и объяснение осталось в stderr,
   * который выбрасывался, потому что ошибка уже была записана из потока.
   *
   * В окно не попадает: это для журнала прогонов.
   */
  diagnostics?: string;
}
