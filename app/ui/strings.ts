...
const ru = {
  ...
  trayRunSetup: 'Пройти настройку заново…',
  ...
  localModel: 'Своя модель вместо подписки',
  localModelHint: 'Для мощных компьютеров. Запустите у себя Ollama, LM Studio или llama.cpp (llama-server --jinja) — Джарвис направит на них Claude Code, вход в аккаунт Anthropic не понадобится. Claude Code всё равно должен быть установлен. Только адрес на этом компьютере или в домашней сети; модель Джарвис не скачивает.',
  localModelCheck: 'Проверить и включить',
  localModelOff: 'Выключить',
  localModelChecking: 'Проверяю — локальная модель может думать до двух минут…',
  localModelOk: 'Модель ответила и умеет вызывать инструменты. Включено.',
  localModelNoTools: 'Модель отвечает, но не вызывает инструменты — агентом она работать не сможет. Для llama.cpp запустите llama-server с --jinja. Не включено.',
  localModelActive: 'Сейчас работает',
...
const en: UiStrings = {
  ...
  localModel: 'Your own model instead of a subscription',
  localModelHint: 'For powerful computers. Run Ollama, LM Studio or llama.cpp (llama-server --jinja) yourself and Jarvis will point Claude Code at it — no Anthropic sign-in needed. Claude Code still has to be installed. Only an address on this computer or your home network; Jarvis does not download the model.',
  localModelCheck: 'Check and turn on',
  localModelOff: 'Turn off',
  localModelChecking: 'Checking — a local model can take up to two minutes…',
  localModelOk: 'The model answered and can call tools. Turned on.',
  localModelNoTools: 'The model answers but does not call tools, so it cannot work as an agent. For llama.cpp, start llama-server with --jinja. Not turned on.',
  localModelActive: 'Now using',
